...
class mdlUsuarios{
	...
	static public function mdlguardarUsuarios($tabla, $datos){
        ...
        $stmt= Conexion::conectar()->prepare("INSERT INTO $tabla(usuario , pass , nombre ,foto , rol) VALUES (:USUARIO_u , :PASS_u , :NOMBRE_u , :FOTO_u , :ROL_u)");
    ...
    static public function mdlEditarUsuarios($tabla , $datos){
        ...
        $stmt= Conexion::conectar()->prepare("UPDATE $tabla SET usuario=:NOM_E , pass=:PASSSER_E , nombre=:NOMUSER_E , foto=:IMG_E , rol=:ROL_E WHERE id=:IDE");


         $stmt->bindParam(":IDE", $datos["idE"], PDO::PARAM_INT);
         $stmt->bindParam(":NOM_E", $datos["nom_userE"], PDO::PARAM_STR);
         $stmt->bindParam(":NOMUSER_E", $datos["nom_usuarioE"], PDO::PARAM_STR);
